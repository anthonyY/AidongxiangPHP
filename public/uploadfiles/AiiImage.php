...
class AiiImage
{
     ...
     * @version 2014-11-24 WZ
     */
    public function setWaterConfig($water)
    {
        $this->water = $water;
        return $this;
    }
    ...
    public function makeThumb($width, $height, $type)
    {
        ...
        {
            $filename = $orgin;
            if (is_file($orgin) && $this->water) {
                $image_info = getimagesize($orgin);
                $width = $image_info[0];
                $height = $image_info[1];
        ...
        elseif (is_file($orgin))
        {
            ...
            $image->makeThumb($orgin, $width, $height, $thumb_path, '#FFFFFF', 99, $type);
            if ($this->water && 1 == $this->water->type)
            {
            ...
            {
                $image->watermark($filename, $filename, 'txt', '', $this->water->water_txt, $this->water->color, $this->water->font, $font_type = 'msyh.ttf', $this->water->pos, $this->water->alpha, $this->water->quality);
            }
            $this->returnImage($filename);
        }
        else
