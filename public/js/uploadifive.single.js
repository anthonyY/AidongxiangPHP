$(function () {
    ...
    $("#one_img_upload2").uploadifive({
        ...
        'onUploadComplete' : function(file, data) {
            setTimeout(function () {
                $('#uploadifive-one_img_upload2-queue').hide();
            }, 2000);
            data = JSON.parse(data);
            getResult_one_img2(data); //获得img的ID
        }
    });
    //添加时候的图片删除
    function getResult_one_img2(img_url){
        console.log(img_url);
        var type = "res_img2";
        var img_id = "image_id2";
        $('#'+type).html('');
        $("#"+type).append('<div id="img_url2" class="outdiv"></div>');
        $("#img_url2").append("<img src="+img_url.path+" onmouseover='del1(this)'  width = '280' height = '210'>");
        $("#img_url2").append("<input value="+img_url.image_id+" name='"+img_id+"' type='hidden'>");
        $("#img_url2").append("<div id='near_img' onmouseout='del3(this)' onclick='$(\"#"+type+"\").html(\"\")' class='innerdiv'>删除</div>");
    }
});
