...
class WxPayDataBase
{
	...
	**/
	 ...
	 */
	public function MakeSign($type = 1)
	{
		//签名步骤一：按字典序排序参数
		ksort($this->values);
		$string = $this->ToUrlParams();
		//签名步骤二：在string后加入KEY
	    if((isset($this->values['trade_type']) && $this->values['trade_type'] == 'APP') || $this->type == "APP" || $type == 2)
	    {
	        if($this->values['appid'] == WxPayConfig::APPID_k)
	        {
		      $string = $string . "&key=".WxPayConfig::KEY_K;
	        }
	        else
	        {
	            $string = $string . "&key=".WxPayConfig::KEY_K_M;
	        }
	    }
	    else
	    {
...
class WxPayResults extends WxPayDataBase
{
     ...
     * @throws WxPayException
     */
	public static function Init($xml, $type)
	{	
		$obj = new self();
		$rs = $obj->FromXml($xml);
		//fix bug 2015-06-29
