...
class WxApi
{
	 ...
	 * **************************************************
	 */
	public function wxOauthUser($open_id,$access_token)
	{
    ...
    public function GetOpenid($baseUrl = '')
    {
        ...
        if (! isset($_GET['code'])) {
            ...
            if(strpos($baseUrl,'code') !== false){
                $baseUrl = str_replace('&'.substr($baseUrl,strpos($baseUrl,'code')),'',$baseUrl);
                $baseUrl = str_replace('?'.substr($baseUrl,strpos($baseUrl,'code')),'',$baseUrl);
            }
            $baseUrl = $baseUrl ? $baseUrl : urlencode('https://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . ($_SERVER['QUERY_STRING'] ? '?' . $_SERVER['QUERY_STRING'] : ''));
            $url = $this->wxOauthUserinfo($baseUrl);
            Header("Location: $url");
            exit();
        } else {
            // 获取code码，以获取openid
            ...
            if(!$data['openid'] || !$data['access_token']){               
                unset($_GET['code']);
                $this->GetOpenid($baseUrl);
            }
            $wxuser = $this->wxOauthUser($data['openid'],$data['access_token']);
            return $wxuser;
        }
    }
    ...
    {
        $data = $this->wxOauthAccessToken($code);
        return $data;
    }
	...
	public function wxMedia($media_id,$data)
	{
	    $wxAccessToken = $this->wxAccessToken ();
	    $url = "http://file.api.weixin.qq.com/cgi-bin/media/get?access_token=" . $wxAccessToken . "&media_id=" . $media_id;
// 	    $this->downAndSaveFile($url,$data[0]);
	    $this->saveMedia($url);
	    return 1;
	}
