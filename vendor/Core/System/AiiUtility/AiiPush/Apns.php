...
class Apns
{
    ...
    function __construct($environment, $providerCertificateFile)
    {
        ...
        if (! is_readable($providerCertificateFile) && ! is_readable(__DIR__ . $providerCertificateFile))
        {
            throw new \Exception("Unable to read certificate file '{$providerCertificateFile}'");
        }
    ...
    public function setRCA($rootCertificationAuthorityFile)
    {
        ...
        {
            throw new \Exception("Unable to read Certificate Authority file '{$rootCertificationAuthorityFile}'");
        }
    ...
    public function send()
    {
        if (! $this->_hSocket)
        {
            throw new \Exception('Not connected to Push Notification Service');
        }
        $sendCount = $this->getDTNumber();
        $messagePayload = $this->getPayload();
        foreach ($this->_deviceTokens as $key => $value)
        {
            $apnsMessage = $this->getBinaryNotification($value, $messagePayload, $key, $Expire = 604800);
    ...
    public function addDT($deviceToken)
    {
        ...
        {
            throw new \Exception("Invalid device token '{$deviceToken}'");
        }
    ...
    public function setBadge($badge)
    {
        if (! is_int($badge))
        {
            throw new \Exception("Invalid badge number '{$badge}'");
        }
        $this->_badge = $badge;
    }
    ...
    public function setCP($name = '', $value)
    {
        ...
        {
            throw new \Exception("Property name '" . self::APPLE_RESERVED_NAMESPACE . "' can not be used for custom property.");
        }
        if ($name) {
        ...
        if (is_array($value)) {
            foreach ($value as $k => $v) {
                $this->_customProperties[trim($k)] = trim($v);
            }
        }
    }
    ...
    public function setExpiry($expiryValue)
    {
        ...
        {
            throw new \Exception("Invalid seconds number '{$expiryValue}'");
        }
    ...
    {
        $sJSONPayload = str_replace('"' . self::APPLE_RESERVED_NAMESPACE . '":[]', '"' . self::APPLE_RESERVED_NAMESPACE . '":{}', json_encode($this->_getPayload()));
 ...
 * $apns->connect(); //连接
 * $apns->addDT('3baa9ca2182913d9a9fe2085c5845a8dfa9b7572d13c1dffc6fee04b16e82d97'); //加入deviceToken
 * $apns->setText('这是一条测试信息2'); //发送内容
