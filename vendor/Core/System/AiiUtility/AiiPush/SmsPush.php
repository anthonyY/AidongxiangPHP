...
class SmsPush {
	...
	private function init() {
		$this->url = SMS_URL;
		$this->serialNumber = SMS_USERID;
		$this->password = SMS_PASSWOED;
		$this->sessionKey = SMS_NUMBER;
	}
	...
	public function pushCollectionDevice($deviceTokens,$content) {
		...
		if($result == 1){
			...
			$this->myfile->putAtEnd(sprintf(STATUS_13001,count($deviceTokens)));
		}
		return $res_arr;
	}
	...
	private function sendMessage($mobile_array,$content){
		...
		if($statusCode != 0){
			$this->myfile->putAtEnd("statusCode:".$statusCode." ".$client->getError());
			return false;
		}else {
			return true;
		}
