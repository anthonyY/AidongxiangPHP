...
class Image
{
	...
	//获取字体长度
	function getFontSize($font,$font_type, $water_txt){
	...
	function watermark($orgin, $target = '', $mark_type = 'txt', $water_img = '', $water_txt = '', $color = '#FF0000', $font = 14, $font_type = NULL, $pos = 9, $alpha = 65, $quality = 85)
	{
		...
		if(! strcmp($mark_type, 'img')) // 如果是图片水印
		{
			...
			{
				$log->err(__CLASS__.'<-->'.__FUNCTION__.$this->_error.'行号为：'.__LINE__.'行');
				return false;
			}
			// 获得水印文件以及源文件的信息
...
//			imagettftext($org_handle, $font, 0, $x, $y, $this->coloral($org_handle, $color), $font_type, $water_txt);
		...
		switch($pos)
		{
			...
			case 10:
				...
				$x = (($org_info[0] - $this->name_str - $this->str)/2) + $this->str;//名字
				$y = ($org_info[1])/4.0 - $mark_info[1]/2;
				break;
			default:
				$x =  ($org_info[0]-$this->name_str-$this->str)/2;//我是
				$y = ($org_info[1])/4.0 - $mark_info[1]/2;
				break;
		}
