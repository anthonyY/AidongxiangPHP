...
use Admin\Controller\Upload\File;
...
// 		$binary_file = file_get_contents($f['tmp_name']);
		...
		$newfile = APP_PATH . '/' . $this->path . ((is_string($this->original) && $this->original != '') ? $this->original . '.' . $fileType : ($this->original ? $f['name'] : $this->newfilename));
		@file_put_contents($newfile, $binary_file);
