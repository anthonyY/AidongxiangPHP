 ...
 * 充值/订单付款
 */
class PaySubmit extends CommonController
{
...
//                                $return_url = 'http://' . $_SERVER['HTTP_HOST'] . $url;
            ...
            case '2': // 2用户中心订单付款
                ...
                if($pay_type == 3 && $user_details->cash < ($order_details->total_cash - $order_details->preferential_amount))
                {
                    $response->status = STATUS_UNKNOWN;
                ...
                if($other_financial)
                {
                    ...
                    $pay_log_model->preferentialAmount = $financial_details->preferential_amount;
                    $pay_log_model->status = 3;
                    $pay_log_id = $pay_log_model->addData();
                    if($pay_log_id)
                    {
                        $financial_model->payId = $pay_log_id;//根据订单id更新财务表的支付id
                        $financial_model->orderId = $order_details->id;
                        $update_financial = $financial_model->updatePayIdByOrderId();
                        if(!$update_financial)
                        {
                            $this->adapter->getDriver()->getConnection()->rollback();//事务回滚
                            $response->status = STATUS_UNKNOWN;
                            $response->description = '更新财务支付记录失败';
                            return $response;
                        }
                    }
                    else
...
//                    }
                        ...
                        $mallPaymentByBalance->payAmount = $order_details->total_cash - $order_details->preferential_amount;
                    ...
                    if($order_details->room_id || $order_details->room_number)
                    {
                        $res_book = $order_model->bookRoom();
                        if($res_book['s'])
                        {
                            $response->status = $res_book['s'];
                            $response->description = $res_book['d'];
                            return $response;
                        }
                    }
                ...
                else
                {
                    if($pay_type == 1)//微信
                    {
                        ...
                        if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false){//公众号支付
                            $pay_para = $this->getWxPayInfo($order_type,$pay_details->out_trade_no,$order_details->total_cash - $order_details->preferential_amount,1);
                        }
                        else
                        {
                            $pay_H5 = $this->getWxPayInfo($order_type,$pay_details->out_trade_no,$order_details->total_cash - $order_details->preferential_amount,2);
                            if($pay_H5 && $pay_H5['return_code'] == 'SUCCESS')
                            {
...
//                                $return_url = 'http://' . $_SERVER['HTTP_HOST'] . $url;
                    ...
                    elseif($pay_type == 4)//4微信扫码支付（PC）V2.0
                    {
                        ...
                        $pay_para = $this->getWxPayInfo($order_type,$pay_details->out_trade_no,$pay_details->cash - $pay_details->preferential_amount,3);
                    }
                    elseif($pay_type == 5)//5App微信支付V2.0
                    {
                        $order_type = $financial_details->transfer_way;
                        $pay_para = $this->getWxPayInfo($order_type,$pay_details->out_trade_no,$pay_details->cash - $pay_details->preferential_amount,4);
                    }
                    elseif($pay_type == 6)//6App支付宝支付
                    {
                        require_once 'AiiLibrary/AliPay/AliPayApi.php';
                        ...
                        $alipay->total_amount = IS_DEBUG == 1 ? $test_amount : ($pay_details->cash - $pay_details->preferential_amount);
            ...
            case 3://3非个人中心订单支付
                ...
                if($pay_type == 3 && $user_details->cash < ($pay_details->cash - $pay_details->preferential_amount))
                {
                    $response->status = STATUS_UNKNOWN;
                    $response->description = "钱包余额不足，请充值";
                    return $response;
                ...
                if($pay_type == 3)
                {
                    ...
                    if($pay_details->cash != 0)
                    {
                        ...
                        $mallPaymentByBalance->password = $user_details->pay_password;
                        $mallPaymentByBalance->payAmount = $pay_details->cash - $pay_details->preferential_amount;
                        $java_pay_result = $mallPaymentByBalance->submit();
                        $respond = $mallPaymentByBalance->getRespCode();
                        if($respond && $respond['respCode'] != 0)
                        {
                    ...
                    foreach ($financial_list['list'] as $item) {
                        ...
                        if(in_array($item->transfer_way,array(2,3)) && $item->user_type == 2) {//是订单,就要增加商家对应的金额
                            ...
                            if($order_details->room_id || $order_details->room_number)//调用订房接口
                            {
                                $res_book = $order_model->bookRoom();
                                if($res_book['s'])
                                {
                ...
                else //不是余额支付
                {
                    if($pay_type == 1)//微信
                    {
                        ...
                        if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false){//微信公众号支付
                            $pay_para = $this->getWxPayInfo($order_type,$pay_details->out_trade_no,$pay_details->cash - $pay_details->preferential_amount,1);
                        }
                        else
                        {
                            $pay_H5 = $this->getWxPayInfo($order_type,$pay_details->out_trade_no,$pay_details->cash - $pay_details->preferential_amount,2);
                            if($pay_H5 && $pay_H5['return_code'] == 'SUCCESS')
                            {
//                                $url = $this->url()->fromRoute('wap-order', array(
//                                    'action' => 'successfulPayment'
