...
function get_app_info($app = 'renren') {
    ...
    if ($app == 'renren') {
        ...
        $result['openid'] = $_SESSION['qq_token']['openid'];
        $result['face'] = $result['figureurl_qq_2'];
        $result['gender'] = $result['gender'];
        $result['birthday'] = '';
    }elseif($app == 'weixin')
    {
        global $weixin;
        $WXresult = $weixin->wxGetUser($_SESSION['wx_token']['openid'], $_SESSION['wx_token']['access_token']);
        $result['name'] = $WXresult['nickname'];
        $result['openid'] = $_SESSION['wx_token']['openid'];
        $result['unionid'] = $WXresult['unionid'];
        $result['face'] = $WXresult['headimgurl'];
        $result['sex'] = $WXresult['sex'];
        $result['birthday'] = '';
    }
    return $result;
}
