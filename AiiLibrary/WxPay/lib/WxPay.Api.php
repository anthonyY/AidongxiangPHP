...
class WxPayApi
{
	 ...
	 * @param int $timeOut
	 * @param int $is_merchants 是否商家端支付 1是 0否
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if($inputObj->GetTrade_type() == "APP")
		{
			...
			$inputObj->SetAppid(WxPayConfig::APPID_k);//开发平台账号ID(用户端)
		...
		if($inputObj->GetTrade_type() == "MWEB") {
			$inputObj->set_scene_info(WxPayConfig::WAP_URL,WxPayConfig::WAP_NAME);
		}
	 ...
	 * @param int $timeOut
	 * @param $type  1公众号 2 APP
	 * @throws WxPayException
	 * @return 成功时返回，其他抛异常
	 */
	public static function refund($inputObj, $timeOut = 6,$type=1)
	{
		...
		if($type==1)
		{
		    ...
		    $inputObj->SetAppid(WxPayConfig::APPID_k);//公众账号ID
		    $inputObj->SetMch_id(WxPayConfig::MCHID_k);//商户号
		}
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		...
		$result = WxPayResults::Init($response,$type);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function refundQuery($inputObj, $timeOut = 6,$type)
	{
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		if($useCert == true){
			...
			if(PAY_TYPE == 1)//公众号支付
			{
			    curl_setopt($ch,CURLOPT_SSLCERT, __DIR__."/".WxPayConfig::REFUND_SSLCERT_PATH);
			    curl_setopt($ch,CURLOPT_SSLKEY, __DIR__."/".WxPayConfig::REFUND_SSLKEY_PATH);
			}
			else //APP支付
			{
