...
class LtCaptchaImageEngine
{
    ...
    protected function WriteText($text, $fontcfg = array())
    {
        ...
        for($i = 0; $i < $length; $i++){
            ...
            if($this->shadowColor){
                $coords = imagettftext($this->im, $fontsize, $degree, $x + $this->conf['scale'], $y + $this->conf['scale'], $this->GdShadowColor, $fontfile, $letter);
            }
    ...
    protected function WaveImage()
    {
        ...
        for($i = 0; $i < ($this->conf['width'] * $this->conf['scale']); $i++){
            imagecopy($this->im, $this->im, $i - 1, sin($k + $i / $xp) * ($this->conf['scale'] * $this->Xamplitude), $i, 0, 1, $this->conf['height'] * $this->conf['scale']);
        }
        // Y-axis wave generation
        $k = rand(0, 100);
        $yp = $this->conf['scale'] * $this->Yperiod * rand(1, 2);
        for($i = 0; $i < ($this->conf['height'] * $this->conf['scale']); $i++){
            imagecopy($this->im, $this->im, sin($k + $i / $yp) * ($this->conf['scale'] * $this->Yamplitude), $i - 1, 0, $i, $this->conf['width'] * $this->conf['scale'], 1);
        }
    }
    ...
    protected function ReduceImage()
    {
        ...
        imagecopyresampled($imResampled, $this->im, 0, 0, 0, 0, $this->conf['width'], $this->conf['height'], $this->conf['width'] * $this->conf['scale'], $this->conf['height'] * $this->conf['scale']);
        imagedestroy($this->im);
        $this->im = $imResampled;
    }
}
